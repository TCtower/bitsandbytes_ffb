...
def supports_igemmlt(device: torch.device) -> bool:
    ...
    FFB optimization: This function should not be queried on every forward pass. Instead, it should be called once
    during model initialization and the result should be cached.
       Current Status: Simply returns True for all devices as we are using AMPERE GPUs
       TODO: Implement the logic to check at the beginning and cache the result
    """
...
class MatMul8bitLt(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, A, B, out=None, bias=None, state=MatmulLtState, stream=None):
        ...
        stream_class = GlobalCudaStream.get_instance()
        ...
        if coo_tensorA is not None and subA is not None:
            used_fp16 = True
            with torch.cuda.stream(stream_class.get_cuda_stream()):
                sub_out = torch.matmul(subA, state.subB)
        ...
        # 4. Mixed-precision decomposition matmul
        if used_fp16 == True:
            stream_class.get_cuda_stream().synchronize()
            output += sub_out
...
def matmul(
    ...
    return MatMul8bitLt.apply(A, B, out, bias, state, stream)
